...
session_start();
    ...
    $username_rec = $_POST['uname'];
    $password = $_POST['pass'];

    $error = array();

    $q = "SELECT * FROM reception WHERE username='$username_rec' AND password='$password'";
    $r= mysqli_query($connect,$q);

    $row = mysqli_fetch_array($r);

    if (empty($username_rec)) {
     ...
     if(count($error) == 0){
        $query = "SELECT * FROM reception WHERE username='$username_rec' AND password='$password'";
        $result = mysqli_query($connect,$query);
        if (mysqli_num_rows($result)) {
                $_SESSION['reception']= $username_rec;

                header("Location:reception/index.php");
            }
            else{
                $error['reception'] = "Invalid Username or Password";
            }
        }
