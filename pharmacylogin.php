...
session_start();
    ...
    $q = "SELECT * FROM phamacy WHERE username='$username' AND password='$password'";
    ...
    if (empty($username)) {
     ...
     if(count($error) == 0){
        $query = "SELECT * FROM phamacy WHERE username='$username' AND password='$password'";
        $result = mysqli_query($connect,$query);
        if (mysqli_num_rows($result)) {
                $_SESSION['phamacy'] = $username;
